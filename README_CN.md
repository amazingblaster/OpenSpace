...
**🖥️ [My Daily Monitor](examples/my-daily-monitor/README.md)** — OpenSpace 赋能你的 Agent 完成大规模系统开发。这个拥有 20 多个实时仪表盘面板的个人行为监控系统位于 [`examples/my-daily-monitor`](examples/my-daily-monitor)，完全由 Agent 构建——通过 OpenSpace 从零进化出 60 多项 Skill，展示了自主端到端软件开发能力。
...
- `audit_only`：只记录 job、packet、decision 和 admission，不写入 Skill。
- `fix_only`：显式/手动 FIX job（例如 MCP `fix_skill`）可在完成 evidence、admission、staged authoring、validation 和 commit 后提交。DERIVED 和 CAPTURED action 会记录为 policy-blocked candidate/proposal，不会直接提交，也不会自动复审。
...
│   ├── application.py                    # OpenSpace/OpenSpaceConfig 公共 facade；生命周期委托给 runtime
...
│
├── apps/
│   ├── dashboard/                        # 仪表盘 UI（React + Tailwind）
...
架构门禁、以及 `tests/unit/` owner 级单元测试。`tests/contracts/` 和

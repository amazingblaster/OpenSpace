...
class EvolutionValidator:
    ...
    def _validate(
        ...
        *,
        ...
        if not failures and self.semantic_enabled and self.semantic_validator is not None:
    ...
    def _run_semantic_validator(
        ...
        *,
        ...
        try:
            ...
            if reviewer is None:
                return outcome, semantic_warnings
            if callable(reviewer):
                response = reviewer(authoring, validator_packet, decision, admission)
            else:
                method = getattr(reviewer, "validate", None)
                if not callable(method):
                    return outcome, semantic_warnings
                response = method(authoring, validator_packet, decision, admission)
        except Exception as exc:

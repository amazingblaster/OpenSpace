...
class SkillEvolver:
    ...
    async def _run_evolution_loop(
        ...
        for iteration in range(_MAX_EVOLUTION_ITERATIONS):
            ...
            if self._llm_client.model_response_has_api_error(model_response):
                logger.warning(
                    "Evolution LLM returned an API error followup (iter %s)",
                    iteration + 1,
                )
                return None
            ...
            if has_tool_calls and evolution_tools and not is_last:
                ...
                stop_reason = tools_result.stop_reason
    ...
    async def _apply_with_retry(
        ...
        *,
        ...
        for attempt in range(_MAX_EVOLUTION_ATTEMPTS):
            ...
            try:
                result = await self._llm_client.call_model_with_fallback(
